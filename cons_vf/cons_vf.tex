...
\item Let $\mathbf{F}$ be a $C^1$ vector field, and let $X$ be open and connected. If $D\mathbf{F}$ is not $\answer{symmetric}$ then $\mathbf{F}$ is not $\answer{conservative}$.
\end{itemize}
