...
\[
\left(\dfrac{\partial f}{\partial x}, \dfrac{\partial f}{\partial y}, \dfrac{\partial f}{\partial z}\right) = \left(2xy,x^2+z+2y,y+\cos(z)\right)
\]
...
&=\sin(z)+C
\end{align*}
Where $C$ is a constant. Thus, any potential function would have the form $f(x,y,z) = x^2y+yz+y^2+\sin(z)+C$. Choosing $C=0$, we have a specific potential function $f(x,y,z)= x^2y+yz+y^2+\sin(z)$.
\end{explanation}
\end{example}
...
\[
\left(\dfrac{\partial f}{\partial x},\dfrac{\partial f}{\partial y}\right) = (-y,x)
\]
...
\begin{theorem}
Let $\mathbf{F}:X\subset\mathbb{R}^n\rightarrow \mathbb{R}^n$ be a $C^1$ vector field, and let $X$ be open and connected. If $D\mathbf{F}$ is not symmetric then $\mathbf{F}$ is not conservative.
\end{theorem}
...
\begin{problem}
For each of the given vector fields, find a potential function (if one exists). If there is no potential function, write ``DNE''.
