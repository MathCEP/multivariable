...
%These problems are all from webwork. Some of them might be ours, but certainly a lot of them can be combined/simplified, so should rewrite these regardless.
...
%First two problems are from Colley - can easily replace with our own versions. The third is from the topology handout, so I assume it's ours.
...
%This is from the topology handout, so I assume is ours.
